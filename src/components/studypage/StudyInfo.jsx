...
const StudyInfo = ( { studyData } ) => {
    ...
    const { title, username, description, profileImage, userId } = studyData;
    console.log(studyData);

    // 스터디 삭제
    const handleDelete = async () => {
        try {
            const response = await api.delete(`/study/${studyData.id}`);
            if (response.status === 204) {
                navigate("/study");
                closeModal();
                showAlert("정상적으로 스터디가 삭제되었습니다.");
            }
        } catch (error) {
        ...
        <div className="flex justify-between items-start w-full mb-16">
            <div className="flex flex-col">
                <div className="flex items-center mb-6">
                    ...
                    <div className="mr-4 text-3sm">{description}</div>
                    {userInfo?.userName === username && (
                        <div className="flex justify-center mt-2">
                            <button className="mx-1">
                                <MdEdit
                                    className="hover:text-blue-400"
                                    onClick={() => navigate(`/study/${studyData.id}/edit`)}
                                />
                            </button>
                            <button className="mx-1">
            ...
            <Modal style="w-120 text-center">
                ...
                <div className="text-xs text-gray-500">
                    삭제하면 더 이상 이 스터디를 풀던 사람들이 이용할 수 없습니다.
                </div>
